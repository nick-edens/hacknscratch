...
public class Scratch extends Sprite {
	...
	public var stageIsContracted:Boolean = false; // true when the stage is half size to give more space on small screens
	public var stageIsMed:Boolean = true; // true when the stage is half size to give more space on small screens
	public var stageIsLarge:Boolean = false; // true when the stage is half size to give more space on small screens
...
//		}
	...
	private function setMedStageMode(flag:Boolean):void {
		stageIsMed = flag;
		stageIsLarge = false;
        ...
        private function setLargeStageMode(flag:Boolean):void {
                stageIsLarge = flag;
                stageIsMed = false;
                stageIsContracted = false;
                stagePart.updateRecordingTools();
	...
	protected function updateLayout(w:int, h:int):void {
		...
		if (editMode) {
			...
			if (stageIsContracted) {
				stagePart.setWidthHeight(320 + extraW, 180 + extraH, 0.25);
			} else if (stageIsMed){
				stagePart.setWidthHeight(640 + extraW, 360 + extraH, 0.5);
			} else if (stageIsLarge){
			...
			var pad:int = (w > 550) ? 0 : 0; // add padding for full-screen mode
			var scale:Number = Math.min((w - extraW - pad) / 1280, (h - extraH - pad) / 720);
			scale = Math.max(0.01, scale);
			//Nick
			//var scaledW:int = Math.floor((scale * 1280) / 4) * 4; // round down to a multiple of 4
			var scaledW:int = Math.floor((scale * 1280) ) ; // round down to a multiple of 4
			scale = scaledW / 1280;
	...
	protected function addFileMenuItems(b:*, m:Menu):void {
        ...
        public function showStageMenu(b:*):void {
                var m:Menu = new Menu(null, 'More', CSS.topBarColor(), 28);
                m.addItem('Small stage layout', toggleSmallStage, true, stageIsContracted);
                m.addItem('Med stage layout', toggleMedStage, true, stageIsMed);
                m.addItem('Large stage layout', toggleLargeStage, true, stageIsLarge);
	...
	public function toggleMedStage():void {
		setMedStageMode(!stageIsMed);
	}

	public function toggleLargeStage():void {
		setLargeStageMode(!stageIsLarge);
	}
