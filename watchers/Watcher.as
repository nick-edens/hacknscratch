...
public class Watcher extends Sprite implements DragClient {
	...
	// Nick
	private const labelFontSize:int = 24; // This is the size of the label for the floating variables on the stage not the size of the value itself or the container

	private const format:TextFormat = new TextFormat(CSS.font, labelFontSize, 0, true);
	...
	private function adjustReadoutSize():void {
		...
		frame.h = (mode == NORMAL_MODE) ? 50 : 70; // this is setting the height of the grey box itself that the variable value and label is in
		frame.setWidthHeight(frame.w, frame.h);
		readout.height = 2000; //  this sets the actual orange variable value height. I have no idea what sort of units this is in as 2000 is an insane number to have to put here. Not px for sure.
